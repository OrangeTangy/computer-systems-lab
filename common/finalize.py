...
files={str(p.relative_to(R)).replace('\\','/'):hashlib.sha256(p.read_bytes()).hexdigest() for p in R.rglob('*') if p.is_file() and not any(part in ['.git','build','__pycache__','scratch'] for part in p.relative_to(R).parts) and p.name!='sha256.json'}
